...
v0.4, which is the version their fitted belief arrives at. Two candidates, both
introduced there and leaned on harder afterwards: an iterative Sinkhorn/IPF fit
that warm-starts from the previous position and need not land where a
from-scratch fit lands, and a determinized search ($\mathtt{det}{=}12$) drawing
from an RNG that our fresh process re-seeds at every single decision.
