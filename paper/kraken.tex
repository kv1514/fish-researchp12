...
interval and withdraws one conclusion. Across the whole project, forty-four
...
is the engine's own way of resolving ownership, not waste. \textbf{A measured
...
actually differ, and it is the channel we lose.
...
interval covers the $+2.3466$ above.
...
\midrule
games & $10{,}000$ & $1{,}200$ \\
our margin (sets/game) & $\mathbf{+2.3466}$ & $\mathbf{-0.6200}$ \\
...
restated, and it is the row the reversal is made of. Since $57\%$ of the $+2.3466$ is declaration
...
$+2.3466$), which is what makes the other column believable rather than a
harness artifact.
...
& our margin & games \\
\midrule
our arbiter, \emph{stateless} bridge (published) & $+2.3466$ & $10{,}000$ \\
...
hit $52.98\%$. After the v1.2 programme (P43, Appendix~\ref{app:prereg}) put
three registered candidates at that $1.95$ points and moved none of them, the
obvious next step was to find a fourth. What follows is the attempt, and it
ends by withdrawing the candidate rather than registering it.
...
ones we go on to ask into). \textbf{We sit on completed half-suits
because we do not know we own them, not because we cannot place the split.}
That is not what the $95.3\%$ allocation-error figure invites a reader to
assume --- that figure counts declarations we made and got wrong, not the ones
we never made --- and it moves the target from splitting to ownership
inference.
...
P44 shot down what the exploratory analysis pointed at.
...
out to find. The latency is real and expensive; it is not caused by an
inability to place a split, because the arm built on that premise could not
find enough positions to fire in; and it is not fixable by asking differently,
because the arm built on that premise made things worse while improving the
metric it targeted. What is left is the ownership inference itself --- the
$0.3120$ --- and nothing in this paper has attacked that directly. We do not
have a candidate for it, and we say so rather than nominating one.
...
3 & a pre-play convention in the rules & belief says yes three times, the duel says no; \emph{speaking} is the entire cost \\
...
figure in this paper (including $+2.3466$ against v0.7 and the $+0.104$
lookahead result) stands exactly as written.
...
refused it, at a cost the belief-space instrument could not see because
\emph{speaking} is itself the price. We do not have a candidate that survives
both the theorem and that duel, and we say so rather than nominating one.
...
engines measured in \emph{their} arbiter, where our $+2.3466$ becomes a loss,
...
\textbf{The engine was measured against one from outside the project.} Against
a frozen build of an independently developed C++ engine
\citep{nguyen2026sestina}, over $10{,}000$ games, $+2.3466$ sets a game. The
two systems play different rule dialects on the one clause their author's own
sweep shows matters (out-of-turn declaration), so that figure is a
measurement under this paper's rules and not a ranking of two projects
(\S\ref{sec:res-headline}).

\textbf{And v1.1 shipped nothing.} Seven pre-registered directions at the
residual returned no adoption; the pre-registered thresholds and outcomes for
all forty-four registrations this project has written are in
Appendix~\ref{app:prereg}. Two mechanisms measured there are real and positive
and still do not ship, because the bar was fixed in advance and was not amended
afterwards. That is the discipline this paper is actually arguing for, and it
costs more than it looks: exactly one of the forty-four registrations
produced a change to the deployed engine.
...
be the whole loss. It is also the one item this study explicitly declines to
turn into an arm on the evidence it has, because the analysis that found it is
exploratory and the family of ask-side fixes it first suggested was refuted by
its own kill-check. What a registration here would have to fix in advance is
what the latency is \emph{for}: a completed half-suit is not declared because
the team cannot place the split, so shortening the wait and declaring correctly
are the same problem seen from two ends, and an arm that only shortens the wait
buys wrong declarations at two sets each. The tail shape is the reason to think
it is tractable anyway --- a third of completions carry seven eighths of the
