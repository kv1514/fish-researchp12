...
the bridge was worth $\mathbf{+3.14}$ $[+2.67, +3.61]$ sets a game \emph{to us}
--- more than the entire margin it produced --- and their declaration errors
fall tenfold, from $0.845$ a game to $0.080$, once their engine is allowed to
keep the state its own arbiter lets it keep. Corrected, KRAKEN \textbf{loses} to
that engine by $-0.69$; measured independently inside their arbiter through
their own bot-package protocol, by $-0.62$. Two routes sharing no host, rules or
code agree to within $0.07$ where the published one is out by three. An absolute
...
the scoreboard. The paired contrast prices it: $200$ deals, each played twice on
...
our margin, persistent bridge & $\mathbf{-0.6900}$ \\
\midrule
the bridge was worth, to us & $\mathbf{+3.1400}$ $[+2.6656, +3.6144]$ \\
\midrule
their wrong declarations/game, stateless & $0.8450$ \\
their wrong declarations/game, persistent & $\mathbf{0.0800}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Zero fallbacks and zero unfinished games in both arms. The stateless
arm reproduces the published figure on fresh deals ($+2.4500$ against
...
our arbiter, \emph{persistent} bridge & $-0.6900$ & $200$ \\
...
arms --- an earlier version of this table gave that total, $400$, and so
...
$0.07$ sets a game. The one that does is the outlier, by three. They are also
