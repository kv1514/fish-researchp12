...
$\mathbf{+2.412}$ $[+2.088, +2.736]$ sets per pair and an independently
developed C++ engine \citep{nguyen2026sestina} by $\mathbf{+2.347}$ sets per
game over $10{,}000$ games, under this paper's rule dialect rather than that
engine's. \textbf{Measured inside that engine's arbiter instead, the sign
reverses} ($-0.62$ over $1{,}200$ games), and their whole released ladder
reverses \emph{order} with it. The cause is ours: our bridge is stateless,
replaying the whole public log into a freshly reset agent at every decision,
where their arbiter keeps one agent per seat alive for the deal. Those agree
exactly on their two scripted baselines ($0$ of $661$ decisions differ) and
diverge on every version from v0.4, the release their fitted belief arrives at
--- up to $40.8\%$ of decisions, and asymmetrically, the stateless path
declaring where the persistent one asks twenty times against two the other way.
A cross-engine \emph{absolute} here is therefore a statement about the bridge as
well as the engines, which is the principle this paper had already written down
...
\paragraph{What this does and does not settle.} It is a fact about decisions
rather than about sets: a different move is not automatically a worse one, and
the paired contrast that prices it in sets is a separate measurement. What it
does settle is that our bridge has not been asking their engine the question
their arbiter asks it, for exactly the versions the head-to-head is about. The
paper's own principle, stated in Appendix~\ref{sec:bridges} long before this
was measured --- \emph{an absolute measured through a bridge is a statement
about the bridge as well as about the engines} --- turns out to have been
describing this paper's own headline.

We are not withdrawing $+2.3466$: it is what $10{,}000$ audited games with zero
substituted moves returned, it is reproducible, and the paired contrasts that
share the bridge are unaffected. But it is now known to be a margin against
their engine \emph{as our bridge runs it}, which is not the same policy their
arbiter runs, and a reader should treat it as such.
