...
covers four iterations of an engine for it, \textbf{KRAKEN} --- v0.4, which
built the inference; v0.6, which corrected a rule and re-measured everything
that depended on it; v1.0, which asked where the remaining errors are and
found that the answer had almost nothing to do with card-reading; and v1.1,
which took seven pre-registered runs at that residue and shipped none of them
--- organised throughout around results that contradicted the expectations
motivating them.
...
probability as the defect. The resulting one-parameter correction, pre-registered, and replicated
against the champion at $+0.0907$ $[+0.0555, +0.1260]$, gains on the stronger
base over a single $2{,}000$-pair run
...
\vspace{2pt}
\noindent
Version~1.1 then took \textbf{seven} pre-registered directions at the residue
and closed all seven, shipping nothing. Four never reached a duel because a
futility screen fixed in advance stopped them, saving roughly $9{,}000$ pairs.
Two closures are stronger than a null. Handing a seat the true deal in one
channel at a time shows that knowing it only when you \emph{declare} is worth
$+1.08$ and only when you \emph{ask} $+0.76$, against $+3.41$ for both:
\textbf{$46\%$ of the largest information channel in the game is interaction},
reachable from neither side alone. And the belief-space search provably cannot
resolve the allocation errors that dominate the residue --- its only update
preserves the ratio $M[c,t_1]/M[c,t_2]$ between two teammates at every depth,
every beam width and \emph{every leaf evaluation including a perfect one},
which prices $99.96\%$ of the allocation uncertainty as unreachable exactly
where the errors occur. Finally, an audit of our own instrument found that a
run reported as ``$162$ positions'' is $162$ consecutive plies from
\textbf{eight deals}; every affected interval is restated with deal-clustered
standard errors and a $t$ critical value at the cluster count, one conclusion
is withdrawn, and no duel result moves --- the duel harness had the right unit
from the start.
