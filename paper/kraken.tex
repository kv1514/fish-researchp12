...
reverses \emph{order} with it: in their arbiter each successive release beats us
by more, while in ours their declaration errors climb monotonically with their
own version number, from $4.7\%$ at v0.3 to $20.7\%$ at v0.7. The dialect, the
opponent and our bot package are each excluded as the cause by controls run
inside their engine. A cross-engine \emph{absolute} from this paper is therefore
conditional on the host in a way we can demonstrate and cannot yet explain.
...
The experiments that would separate these are listed in
\path{RESEARCH_FRONTIER.md}; the cheapest is a decision-level comparison of
their own belief against itself, in their arbiter and through our shim at
matched positions, which their repository already carries the instruments for.
Until one lands, the claim this paper makes is the narrow one: two measurements
exist, they disagree in sign and in order, the disagreement is concentrated in
the component that carries most of the margin, and what produces it is not yet
known.
...
\paragraph{The cross-engine margin is arbiter-dependent, and we cannot yet say
why.} The largest limitation this paper acquired late, and the one a reader
should weigh hardest. \S\ref{sec:otherarbiter} reports the same two engines
measured in \emph{their} arbiter, where our $+2.3466$ becomes a loss --- and
their whole released ladder reverses order with it. Three reconciliations are
excluded by controls: the rule dialect and the opponent, by experiments inside
their engine involving nothing of ours; and our bot package, by its beating
their v0.3 inside their own arbiter. What remains is our arbiter, our bridge or
the pair, and $57\%$ of the margin sits in the component that moves.

The specific thing left unexplained is not a level but an \emph{order}: their
successive releases misdeclare monotonically more against us and monotonically
less at home, and improving strength does not produce that shape. A reader
should treat every cross-engine \emph{absolute} here as conditional on the host.
The paired contrasts are unaffected --- an absolute measured through a bridge is
a statement about the bridge as well as about the engines, whereas a contrast
whose arms share the bridge is not (Appendix~\ref{sec:bridges}) --- and that
distinction is doing real work in this paper rather than being recited.
