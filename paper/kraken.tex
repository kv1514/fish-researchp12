...
reverses} ($-0.62$ over $1{,}200$ games); the rule dialect and the opponent are
both excluded as the cause by experiments run inside their engine, so a
cross-engine \emph{absolute} is conditional on the host in a way we can
demonstrate and cannot yet explain.
...
It is stated as open rather than resolved because the reverse cell is not a
clean control. Our champion reaches their arbiter through a package that
answers off-turn declaration polls only on certainties --- our policy has no
out-of-turn behaviour to offer, having never been developed against the channel
--- so the two versions steer into different positions and the opponent in the
second column is \emph{a} realisation of our champion rather than the one in
the first. The measurement is also in tension with the mechanism proposed
earlier in this section, that our ask certifies ``at least one other card of
this half-suit'' and a later take against us discharges it: that is a property
of our \emph{asking} rather than of our arbiter, so it predicts the same errors
in their arbiter, and they are not there.

Three experiments would separate these, and none of them is expensive; they are
listed in \path{RESEARCH_FRONTIER.md} with the first --- their arbiter, our
dialect, our champion as the opponent --- holding everything fixed but the
arbiter and the bridge. Until one lands, the claim this paper makes is the
narrow one: two measurements exist, they disagree in sign, the disagreement is
concentrated in the component that carries most of the margin, and what
produces it is not yet known.
...
why.} The largest limitation this paper acquired late.
\S\ref{sec:otherarbiter} reports the same two engines measured in
\emph{their} arbiter, where our $+2.3466$ becomes a loss. Both measurements are
real; neither is a ranking; and the two obvious reconciliations --- the rule
dialect, and the opponent --- are ruled out by experiments inside their engine
that involve nothing of ours. What remains is our arbiter, our bridge, or the
pair, and $57\%$ of the margin sits in the component that moves. A reader
should treat every cross-engine \emph{absolute} in this paper as conditional on
the host, and should note that the paired contrasts are not affected: an
absolute measured through a bridge is a statement about the bridge as well as
about the engines, whereas a contrast whose arms share the bridge is not
(Appendix~\ref{sec:bridges}). The experiments that would settle it are named
and none is expensive; that they had not been run when this version was
finished is a fact about the schedule and not about their difficulty.
