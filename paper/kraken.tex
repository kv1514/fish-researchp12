...
The provenance is weaker than we would like and is stated rather than assumed.
Their repository is available to us as a single squashed commit, so the
parameter vectors compiled into v0.4, v0.5 and v0.6 cannot be checked against
the ones their published results were generated from; their own manifests cite
commits absent from the snapshot and record a dirty working tree. This
...
\newblock Software repository and technical reports,
  ...
  commit \texttt{d017fbc} for every game reported here. That commit may no
  longer resolve: \citet{nguyen2026sestina} records that the repository's
  history was rewritten after its evaluation. The frozen configuration string
  in \path{external_v07/v07_spec.txt} identifies the build we played.
