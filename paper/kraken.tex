...
(\path{results/perpetual_study_void.json} against
\path{results/perpetual_study_award.json}). The rule reaches only
\path{claim4.forced_claim}, where it changes the split ranking from
...
one candidate split at all (\path{scripts4/rule_bite.py}).}
\label{tab:perpetual}
\end{table}
