...
The reader should weigh the headline accordingly. $57\%$ of $+2.3466$ is
declaration accounting; their declarations are four times worse against us at
v0.7 than at v0.3 while their own arbiter says the opposite; and we cannot
currently say why. We are not withdrawing the number --- it is what
$10{,}000$ audited games with zero substituted moves returned, and the
paired contrasts that share the bridge are unaffected either way --- but a
cross-engine \emph{absolute} from this paper should now be read as
conditional on the host, and the burden of showing it is not an artifact has
not been discharged.
