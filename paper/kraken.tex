...
interval and withdraws one conclusion. Across the whole project, forty-six
...
\texttt{opponent\_gamma}, and which side of the table that cost belongs to is
what the fourth registration (P46, registered and running as this is
written, \S\ref{sec:next}) separates. The
...
residual returned no adoption, and three registrations of the v1.2 programme
(ten arms, each required to clear $+0.15$ against SESTINA and in self-play)
returned none either; two of those arms raised the ask hit rate the
cross-engine table said we lack, and both lost by a third of a set doing it.
The pre-registered thresholds and outcomes for all forty-six registrations
this project has written are in Appendix~\ref{app:prereg}; the forty-sixth
(P46) is registered and running, and no result of it appears here. Two
...
looks: exactly one of the forty-six registrations produced a change to the
deployed engine.
...
more.
\item \textbf{What a fourth registration measures, and what is closed without
one.} P46 (\path{prereg/kraken_v12_belief_grid.md}) is registered and running
as this is written, and no result of it appears in this paper. It measures the
one thing three programmes never measured: the belief about SESTINA's cards.
Every P43 sentence about the opponent model rests on a two-knob difference,
because \texttt{gamma\_team} defaults to \texttt{opponent\_gamma}, so
switching the model off on their three seats also switched it off on our two
teammates. Stage~0 scores twenty cells of the model exponent, on their seats
and on ours, against the dealt truth on frozen champion decisions at two
budgets ($720$ and $2{,}880$ draws), with the Kish ESS recorded per row and
intervals clustered by game; it ships nothing. Stage~1 plays the single-knob
decomposition of P43's model-off arm (off on their seats only; off on ours
only) at $300$ deals on both parities in both populations, plus one upward
cell licensed only by a rule fixed before Stage~0 was run. The registered
prediction is that nothing clears either half of the bar, for a fourth time,
and that the value is the table. Four doors are closed without a registration,
and the record behind each is written into the registration so that the
omission is a decision and not an oversight. The draws budget: the self-play
...
the measurement of \S\ref{sec:ownership-closure}. If P46 returns what it
predicts, the residual half set is not reachable by any single knob this
engine exposes, and the next attempt would have to change what the engine
