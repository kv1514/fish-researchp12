...
What that does \emph{not} establish is where the value goes. The two arms are
level on our own declaration errors, so the cost is not paid in our mistakes,
and this instrument cannot see the rest of the board: it counts what our three
seats declare and discards what the opposing three do. An earlier draft closed
here by calling the mechanism ``a stall that happens to communicate''. That
reads a ledger of one team's declarations as though it were the whole
scoreboard, and it is withdrawn pending the measurement that counts both
sides.
