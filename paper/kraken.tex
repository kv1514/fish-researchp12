...
interval and withdraws one conclusion. Across the whole project, forty-three
...
own kill-check, and no pre-registration is issued.
...
\textbf{So the programme stops here rather than producing a registration.} The
candidate it was heading for --- a $p(\text{dead})$ penalty in the ask objective
--- is refuted by its own kill-check, and the two knobs in that family this
study has already run are consistent with that: \texttt{avoid\_doomed\_asks}
fires only where the posterior is \emph{certain} the ask cannot land, which is
$1.5\%$ of decisions against the $13.79\%$ measured here, and it scored
$-0.0933$ against SESTINA and $-0.0900$ in self-play (P43). What the analysis
licenses instead is a registration in the declaration channel, on the latency
rather than on the accuracy: the accuracy is already $97.59\%$, and the tail
above says the cost is concentrated in a third of completions that sit for ten
plies or more. We have not written that registration, and this section is not
it.
...
all forty-three registrations this project has written are in
...
costs more than it looks: exactly one of the forty-three registrations
