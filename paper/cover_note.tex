...
%
% WHY THIS FILE EXISTS. The report is 171 pages. That length is defensible --
...
{\bfseries\large KRAKEN: Exact Inference Is Not Enough}\\[0.15em]
{\normalsize Opponent models, endgames, and pre-registered evaluation in
six-player Literature --- KRAKEN v1.1}\\[0.35em]
{\small Kausthubh Veldanda \quad$\cdot$\quad accompanying
\texttt{kraken.pdf}, 171 pages}
...
ladder, v0.2 through the external engine, and the ladder came out \emph{backwards}: in my
...
v0.5 & $336$ & $137$ & $40.77\%$ \\
v0.6 & $292$ & $36$ & $12.33\%$ \\
The external engine & $296$ & $84$ & $28.38\%$ \\
\bottomrule
\end{tabular}
\end{center}
...
Forty-eight pre-registrations, written before the runs, each naming its
...
Since the correction, six pre-registered programmes (P43--P48, sixteen
arms) have tried to recover the deficit under $+0.15$ against their engine
\emph{and} in self-play; none cleared. Two arms closed the one metric we trail
on, the ask hit rate, and both lost a third of a set doing it --- a deficit in
a metric is not necessarily one to close, now replicated. The one arm a
belief-level rule licensed is, at confirm, a null against their engine and
$+0.17$ in self-play: the half of the bar that exists to be distrusted.
