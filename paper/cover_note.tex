...
%
% WHY THIS FILE EXISTS. The report is 124 pages. That length is defensible --
...
survives between decisions. \textbf{The bridge was worth $+3.14$ $[+2.67,
+3.61]$ sets a game to me} --- more than the entire margin it produced. Their
declaration errors fall roughly tenfold, from $0.845$ a game to $0.080$, once
their engine is allowed to keep the state its own arbiter lets it keep.

Corrected, KRAKEN \textbf{loses} to that engine by $-0.69$. Measured
...
no deal generator and no line of code agree to within $0.07$ --- where the
...
Three results follow. Residual errors are not failures to \emph{find} cards:
...
reverse. Exact best responses over information sets put the deployed policy's
endgame exploitability at $+0.4405$ over $305$ one-half-suit-live positions.

\section{How the claims are held to account}

Forty-two pre-registrations, written before the runs, listing the prediction
and the ship bar; Appendix~A tabulates every one on the same footing,
\emph{including} the ones whose predictions failed, and a test checks that
table against the directory in both directions. Nothing ships on a difference
below $\pm0.15$ sets per game. Every number in the report is pinned by a script
to the JSON artifact that produced it, and that script is verified by
perturbing an artifact and confirming it fails. Cross-engine work is pinned to
...
Which piece of their per-decision state the bridge discards --- their
determinized search's random stream, their tie-breaking stream, or genuine
path-dependence in their belief --- is not yet separated. It matters: the first
two are cheap to fix and would make a stateless bridge honest, and the third is
not fixable from outside their engine at all. The $+3.14$ price rests on $200$
paired deals; the effect is an order of magnitude past its interval, but I
would want more games before quoting it on its own.
