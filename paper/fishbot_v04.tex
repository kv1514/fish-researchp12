...
\begin{quote}
\textbf{Every figure in this section is withdrawn pending recomputation.} The
solver's transposition table keyed nodes on hands, turn, half-suit winners and
belief weights, and \emph{not} on the history. The opponents in it are the
champion, whose action is a function of its whole observation, so two nodes
identical in every key field but reached by different move orders have
different continuations and different values. Merging them returned one
branch's value for the other, and the maximisation read those values --- on the
position that exposed it, the search reported $+0.2500$ for a move a rollout of
the same strategy scores at $+0.7500$, and visited $154$ nodes where the
corrected search visits $12{,}086$. The fault was found by an invariant that
holds with no ground truth at all: a best response may copy the champion at
every information set, so it cannot score below it. Five $m = 2$ positions did.
The champion's own figure below is unaffected --- it is a rollout that never
consulted the table --- but the optimum and the gain are not, and the
disagreement counts in Section~\ref{sec:iidiff} come from the same search. The
numbers are being recomputed; until they are, read nothing here as measured.
\end{quote}
...
The control runs first and gates the rest. Where the belief pins every card
there is nothing hidden left, so the exact value must equal the closed form
$2f - m$. Over $200$ games it does, \textbf{$344/344$}.

\subsection{What the endgame is worth, and what the champion gets}

Across $308$ genuinely hidden positions solved exactly ($13$ skipped for a
belief support above $24$ deals, so $96\%$ coverage):

\begin{center}
\begin{tabular}{lr}
\toprule
exact optimum in the seat on move & $+0.5129$ \\
the champion in the same seat & $+0.0670$ \\
\midrule
exact gain from deviating & $\mathbf{+0.4459}$ \\
\bottomrule
\end{tabular}
\end{center}
...
negative entry in $308$ positions.
...
champion cannot see, it captures $+0.067$ of the $+0.513$ available.
...
$84$ of $273$ agree. Of the $189$ disagreements, $79$ cost \emph{exactly}
zero --- ties broken differently, not errors --- and the rest cost $+0.5267$ on
average. Their shape does not implicate one component: of $96$ costly
ask-versus-ask disagreements, $49$ share the target only, $12$ the card only,
