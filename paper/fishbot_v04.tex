...
what the second factor does, and across the $34$ cells of this study that store
their per-pair differentials --- spanning a belief-space search, a sampling
budget, an opponent-model exponent and a history-dependent penalty, features
with nothing in common but the harness --- it barely moves:
$3.91 \\pm 0.22$, a relative spread of $5.6\\%$ against
$9.0\\%$ for the raw standard deviation. That
comparison is the claim: if the conditional factor were as variable as the raw
standard deviation, the decomposition would explain nothing.

The correlation between $s$ and the raw standard deviation is $+0.862$, and we
...
$I^2 = 61.4\%$, $\hat\tau = 0.150$ --- the same size as the pooled effect. The
A/A study measured no between-run variance at all
(Section~\ref{sec:res-lookahead}), so this is an effect that depends on the deal
population rather than a reason to change estimator.
...
question of whether any of them was in fact resolvable. Of the $31$ cells that
stored their per-pair differentials, rescoring each against its own measured
noise moves the resolvable effect by a median factor of $0.92$ and by $0.60$ in
the quietest, and changes \textbf{no verdict at all}: every cell reported here as
an uninformative null remains one, and every cell that resolved still resolves.
The remaining $72$ cells recorded only a mean and an interval, and nothing can
recover what was not written down --- which is itself the argument for storing
per-pair data, and why v0.4 does.
...
per-position slope \emph{differences} without weights gives $+0.194 \pm 0.160$,
apparently a null. That is a different estimand, not a more careful version of
the same one: a single position's slope is very noisy (they run from $-6$ to
$+4$), and an unweighted average is dominated by positions carrying almost no
contrast in $\Prob(\text{success})$, whereas the reported slopes weight each
position by its own sum of squared centred $\Prob(\text{success})$. A pairing
that changes the weighting is not a pairing. No single position carries it (the largest leave-one-out move
