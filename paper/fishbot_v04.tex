...
layouts --- prove that its natural form is exactly greedy at every depth, and
report the coupled form as a further null whose own screening cell likewise
failed to replicate. We also report several
...
candidates can make such a search non-trivial. The coupled version is then a null
in play, after a screening cell that did not replicate: this is the study's
second false positive, and the more instructive one, because it arrived with an
ablation behaving exactly as its theory predicted
...
\subsection{Belief-space search, and the second false positive}
...
halves were measured. Table~\ref{tab:lookahead} is the whole record.

% BEGIN lookahead-table (generated by scripts4/lookahead_table.py --write)
% END lookahead-table

\paragraph{The screen.} Four cells at 200 pairs, of which one resolved: depth
$3$ at $w = 0.25$, $+0.570$ sets per deal-pair, 95\% CI $[+0.125, +1.015]$. The
supporting pattern looked right, which is the problem. The no-coupling ablation
came in at $+0.070$, exactly the null Proposition~\ref{prop:greedy} predicts, so
the effect appeared to be attributable to the one mechanism the design says is
load-bearing. Depth $2$ sat between depth $1$ and depth $3$ at $+0.345$, which
reads as a dose-response in the depth. A reviewer handed those four rows would
have had a coherent story with a mechanism attached.

\paragraph{The retest.} On 500 fresh deals the same configuration returned
$-0.044$, 95\% CI $[-0.321, +0.233]$. That cell resolves effects down to $0.48$
sets per pair, so it is not merely a wider interval containing the screen's
estimate: it excludes the screened value. The $+0.570$ was noise.

Two things about the supporting pattern are worth stating, because both were
part of what made the screen persuasive and neither survives. The apparent
dose-response in depth was assembled from three cells whose intervals all
overlap each other, so it was never evidence of a trend. And the $w = 0.60$ cell
had already come in at $-0.080$: a genuine effect at $w = 0.25$ that vanishes at
$w = 0.60$ is possible --- v0.3's ask terms did show an inverted U --- but it is
also exactly what four draws from a null distribution look like when one is
allowed to pick the largest.

\paragraph{What this is the second instance of.} Section~\ref{sec:res-replication}
records a screening study over twenty-five cells producing precisely one interval
that excluded zero, which failed to replicate twice. This is the same event on a
smaller pool: four cells, one apparent winner, gone on retest. The base rate is
what one should expect --- a nominal 95\% interval per cell means roughly one
false positive per twenty cells under a complete null, and we have now run enough
cells across two versions to have collected two of them. Neither was caught by a
multiplicity correction. Both were caught by one more run on fresh seeds, which
cost under an hour each time.

The methodological point we would press is narrower than ``correct for
multiplicity''. What made both of these dangerous was not the interval; it was
that each arrived with a \emph{mechanism}. The first came with two
independently-motivated changes that stacked, which is a real thing that had
happened before in this project. The second came with an ablation that behaved
exactly as its theory predicted. A plausible mechanism is not evidence, and the
cheap defence is not a smaller $\alpha$ --- it is that the winner, and only the
winner, gets re-measured.

\paragraph{What survives.} Proposition~\ref{prop:greedy} does not depend on any
of this. It says that a possession-chain search over a belief whose transition
edits only the asked card's row is greedy, exactly, at every depth --- so the
obvious version of this idea is a no-op by construction rather than by
measurement. That is a structural statement about why search keeps failing in
this game, and it is the part of Section~\ref{sec:lookahead} we would keep.

What we cannot say is that belief-space search does not work. The retest bounds
the effect of \emph{this} configuration --- possession-chain value, one sweep of
quota coupling, beam 4, depth 3, entering additively --- below about $0.48$ sets
per pair. It says nothing about a lookahead with a different objective at the
leaf, a converged rather than single-sweep coupling, or a horizon past the end of
one possession. The design space is not exhausted; one point in it is.

The implementation ships disabled (\texttt{w\_lookahead} defaults to $0$), for
the same reason the tablebase ships enabled despite changing no decisions: it
costs nothing when off, it is tested, and the next person to ask this question
should not have to build it again.
...
the variance diagnosis leaves open, and a null: $-0.044$ sets per deal-pair over
$500$ duplicate deals, 95\% CI $[-0.321, +0.233]$, against a $200$-pair screening
cell that had reported $+0.570$ and excluded zero. Recorded at length in
Section~\ref{sec:res-lookahead} because it is the study's second false positive
and the more instructive of the two: it arrived with an ablation that behaved
exactly as its own theory predicted, which is a more persuasive kind of wrong
than a bare interval. The implementation ships disabled.
...
and Section~\ref{sec:res-lookahead} reports it as a null at $500$ pairs, after a
screening cell that did not replicate. The negative result is bounded and
narrow: it covers possession-chain value, a single sweep of quota coupling, beam
$4$, depth $3$, entering additively. Proposition~\ref{prop:greedy} says the
