...
The branching factor of a real position averages $8$, so the tree is bounded by
a beam of $4$ candidates per ply, ranked by immediate $p$. At depth $3$ that is
at most $84$ node expansions, and measured over real positions the whole bonus
vector costs a median of about $14$\,ms per decision on the contended host of
Section~\ref{sec:counting} --- roughly a $15\%$ addition to a $100$-ms
posterior, and the one part of v0.4 whose cost is a search rather than an
inference.
