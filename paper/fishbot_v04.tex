...
non-trivial. Whether the coupled form helps we report as \emph{unresolved}, and
that turned out to be the more useful finding: three runs of the identical
configuration returned $+0.570$, $-0.044$ and $+0.386$ sets per deal-pair, which
is more disagreement than their own intervals allow ($Q = 7.03$, $p = 0.030$,
$I^2 = 71.5\%$). The variance that separates them is not within-run sampling but
a between-run component of $\tau = 0.268$ sets per pair that no interval in this
paper models, and which makes the minimum detectable effect of every single-cell
result here optimistic. We also report several
...
candidates can make such a search non-trivial. Whether the coupled version helps is
\emph{unresolved}, and the attempt to resolve it produced a methodological
result we consider more valuable than the search would have been: three runs of
one configuration disagree beyond their own intervals, exposing a between-run
variance component that no confidence interval in this paper accounts for
...
Continuations are therefore beamed to the best $4$ candidates by immediate $p$.
The root ply is \emph{not} beamed, because the policy needs a bonus for every
candidate it is ranking, so the cost is $n\,(1 + b + b^2) = 21n$ node expansions
rather than the $b + b^2 + b^3 = 84$ a uniformly-beamed tree would give. At the
median $n = 45$ that is about $945$ expansions, and the measured median of about
$14$\,ms per decision on the contended host of Section~\ref{sec:counting} is
consistent with that rather than with $84$ nodes, which at the same per-node cost
would be nearer $1.4$\,ms.
...
$\sim\!100$\,ms rows, which are all $512$-draw. So the bonus adds roughly $44\%$
to a decision, not the $15\%$ an earlier draft claimed by comparing against a
budget these runs never ran at. The wall clock agrees and is harsher: the
champion-family cells average about $2.1$\,ms per action while the depth-3
lookahead cell runs $319.70$\,s over $42{,}373$ actions, i.e. about $7.5$\,ms per
action across both sides. This is the one part of v0.4 whose cost is a search
rather than an inference, and it is not cheap.
...
The variance the intervals miss sits \emph{between} runs, and the
DerSimonian--Laird estimate of it is $\tau = 0.268$ sets per deal-pair.

Pooling all 1200 pairs therefore gives two answers, and only one of them is
usable:
\begin{center}
\begin{tabular}{lll}
\toprule
pooling & estimate & \\
\midrule
fixed effect   & $+0.226$ $[+0.041, +0.411]$ & excludes zero --- \emph{and is invalid here} \\
...
is precisely what $Q$ rejects. The random-effects interval, which adds $\tau^2$
to every cell, is the one that reflects where a fourth run might land, and it
includes zero. \textbf{The effect is unresolved.} We cannot report this search as
working, and we cannot report it as a null either.

\paragraph{Why this matters beyond one experiment.} Section~\ref{sec:res-ladder}
already recorded this phenomenon informally: the headline comparison was run
three times and the estimates spanned $+1.85$ to $+2.44$, ``wider than their
individual intervals suggest''. We attributed that to the ordinary observation
that an interval covers the deals actually played. The measurement here says the
effect is larger and more structural than that reading allowed. If
$\tau \approx 0.27$ sets per pair is a general property of this harness rather
than an artefact of three cells --- and three cells estimate $\tau$ very poorly,
so this is a signal and not a settled number --- then the minimum detectable
effect table of Section~\ref{sec:eval} is optimistic for \emph{every} single-cell
result in this paper. At $n = 500$ the within-run MDE is $0.48$; with a
between-run $\tau$ of $0.27$ the standard error of a fresh run's estimate is
$\sqrt{0.48^2/(2.8)^2 + 0.27^2} \approx 0.32$ rather than $0.17$, which nearly
doubles the effect a single run can be trusted to resolve.

The effects this paper reports as its main results are large enough to survive
that --- the opponent model is $+1.9$, the headline ladder $+1.85$, and the
value-objective failure $-7.4$, all far above any plausible $\tau$. What it
threatens is the interpretation of the \emph{small} cells: several of
Table~\ref{tab:nulls}'s rows sit at $\pm 0.3$, and a between-run standard
deviation of $0.27$ means those rows carry even less information than their
already-wide intervals suggest. That strengthens rather than weakens the
paper's existing warning about them: they are ``not shown to matter at this
budget'', and the budget is smaller than it looked.

\paragraph{Why $\tau$ could not be estimated better than this.} Two properties
of the harness, both verified in the code, are why this section reports a
variance component inferred from three run \emph{means} on two degrees of
freedom rather than measured properly. First, \texttt{Result.to\_dict} in
\texttt{fish4/match.py} persists only aggregates --- pair score, mean, interval
--- so \texttt{results/v04\_duels.jsonl} contains no per-pair differentials, and
the within-run and between-run components cannot be separated from the archive
after the fact. Second, and more awkwardly, \emph{the harness cannot measure
itself}: in \texttt{\_one\_deal} both games of a pair share the deck, the start
seat and the agent seed, and the two team assignments are mirror images, so when
\texttt{spec\_x} equals \texttt{spec\_y} the two games are byte-identical and the
pair differential is exactly zero by construction. A true A/A run therefore
reports no variance at all, for every pair, however many are played. Both are
fixable --- log the differentials, and add a seed salt so a policy can be played
against a differently-seeded copy of itself --- and until they are, any estimate
of $\tau$ in this project rests on the handful of configurations that happen to
have been run more than once.

\paragraph{What we would conclude.} The correct experiment is not another 500
pairs on fresh seeds --- that adds a fourth estimate to a set that already
disagrees. It is either a single run large enough that $\tau$ is negligible
beside it, or a design that removes the between-run component by pairing across
seed streams the way duplicate deals pair across teams. We did not run either,
and we would rather leave the question open than settle it by choosing which of
three runs to believe.
...
the part of Section~\ref{sec:lookahead} we would keep. Its empirical shadow is
the no-coupling cell, which is a null at both 200 and 500 pairs
($+0.070$ and $+0.112$, both intervals comfortably containing zero) exactly as
...
$+0.278$, 95\% CI $[-0.083, +0.638]$. We could have reported this as a success
or as a failure by choosing a run, and the reason we can report neither ---
a between-run variance of $\tau = 0.268$ sets per pair --- is in
Section~\ref{sec:res-lookahead}. The implementation ships disabled.
...
and Section~\ref{sec:res-lookahead} leaves it unresolved, because three runs of
it disagree by more than their intervals allow. Settling it needs a design that
removes the between-run component rather than another run on fresh seeds, and
