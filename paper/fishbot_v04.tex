...
\texttt{results/infer\_position\_stats.json}, which stored the \emph{quota}
...
holder and \texttt{\_apply\_claim} awards the half-suit to the opponents. There
is no spite-null available: a certain $-1$ cannot be converted into a $0$.
Hence the opponents take all $m - f$ whatever the mover does.
...
saying the product of marginals is not the joint. Under the baseline null rule
the forced-claim ranking is $\Prob(\text{exact}) + \Prob(\text{ours}) - 1$, so
the two carry equal weight; and their difference is read as ``ours but wrongly
split'', a quantity subtracted across two different distributions and therefore
able to come out negative --- which it did, and a clamp hid it.
...
(\texttt{results/claim\_screen\_check.json}) with a test that fails if a
...
\texttt{jobs/PREREGISTRATION\_combined.md} fixes, before any pair: two blocks
...
\texttt{jobs/PREREGISTRATION\_precision2.md} with a \emph{minimum interesting
...
\texttt{results/ess\_probe.json} --- the term was discarded before use. In the
...
(\texttt{scripts4/greedy\_shadow.py}), the coupled arm departs from the
...
check.} \texttt{check\_paper\_numbers.py} watches over a hundred figures
...
\texttt{settle\_verdict.py} printing the lookahead headline and persisting
...
\texttt{scripts4/unwatched\_claims.py} extracts every number the paper asserts
...
\texttt{pgrep -f "learn\_ask\_objective.py rollout --run v2"}. That matches the
...
\texttt{/proc} and counts a process only when it is a Python interpreter whose
...
above is estimation. \texttt{jobs/PREREGISTRATION\_learned\_weights.md} fixes,
...
(\texttt{results/target\_feature\_fit.json}, $+0.649$ against this
...
\texttt{results/stuck\_claim\_value.json} rather than printed --- which is the
failure \texttt{scripts4/check\_verdicts.py} exists to catch, found here in a
...
and accepting the measured $23.4\%$ null rate on stuck half-suits, would beat
waiting wherever the MAP is right more than $76.6\%$ of the time --- which the
...
\subsection{The claim threshold: the one screen that resolved, re-run}
...
nulls, and it contradicts a documented conclusion: \texttt{fish4/claim4.py}
...
evidence and \texttt{jobs/SCREENING\_DISCIPLINE.md} forbids it from sizing its
own confirmation. \texttt{jobs/PREREGISTRATION\_claim\_threshold.md} fixes, in
...
data; they are the same policy, and \texttt{claim4.py}'s argument survives its
...
numbers appear in no results file: \texttt{perpetual\_study.py} tracked which
...
\texttt{results/perpetual\_study.json} and is checked by
\texttt{scripts4/check\_paper\_numbers.py} against it.
...
\texttt{fish4/exact\_ii} enumerates the deals consistent with the public record
...
$t = +3.37$ over $305$ positions); at $m = 2$ it is suggestive rather than
significant ($r = +0.145$, $t = +1.52$), and the two agree in sign and rough
magnitude. The mechanism is the obvious one: the more the belief leaves open,
the more an exact answer is worth over a heuristic.
...
That reading is an extrapolation, and at $m = 2$ it does not survive
being checked. \S\ref{sec:iibounds} bounds every position in the layer
instead of solving it, and the only quantity measurable on both sides of the
cap is flat across it. The slope above is real where it is fitted and it does
not continue. No corrected figure is offered, and none is claimed to be
needed: what changes is the direction the uncertainty points, not the number.
...
are too loose to decide; \texttt{prereg/ii\_bounds.md} fixed that reading
before the run.
...
\paragraph{Across the cap, the gain is flat (exploratory).} Because the
one-ply gain is computable everywhere and the exact gain is not, it is the only
\emph{matched} instrument spanning the support cap. Splitting at the study's
$24$:
...
($n = 79$); Welch $t = -0.42$. The slope against support over all $248$ is
$-0.00032$ per deal, $t = -1.13$. The rise measured over supports $2$--$24$
peaks around $9$--$60$ and falls away rather than continuing, which if it
holds is a statement about the game: exploitability is largest at
\emph{intermediate} uncertainty, not at maximum uncertainty. When the belief
leaves almost everything open, exact play has little more to work with than
the heuristic does, and the two converge again.

This comparison is \textbf{exploratory} and the pre-registration records it as
such: it was added after the upper bound turned out not to bind, and its bands
were chosen after seeing the support distribution. Its weakness is stated with
it --- $L$ is a lower bound whose looseness is calibrated only where the exact
value is also computed, at supports up to $12$. If the one-ply policy captured
steadily less of the exact gain as support grew, a flat $L$ would be consistent
with a rising exact gain. The gap shows no trend against support
($t = +0.57$ over $100$ positions), which is weak evidence of a fixed offset
rather than evidence of one. What can be said without qualification is the
negative: the claim that the headline is a lower bound rested on a trend, and
the only measurement that spans the cap does not show that trend.
...
almost any team-only declaration fits some deal. Every such claim nulled:
$+0.000$ sets each against $+0.968$ for the rest.
...
\texttt{scripts4/exploitability.py} reports. Over 60 games:
...
\texttt{fish4/bestresponse.py} warns strategy fusion can do to a PIMC
responder.
...
under a third of hidden $m = 1$ decisions. \texttt{fish4/endgame\_ii.py} puts
...
\paragraph{One mechanism, visible in the raw counts.} Over the $1200$ games the
policy produced $446$ nulls against the control's $473$. A null is a claim
with the right team and the wrong split, and the solver enumerates only
declarations true in some consistent deal.

\paragraph{Cross-play: the gain is not exploitation of one opponent.} The
obvious objection is that \texttt{fish4/exact\_ii} best-responds to the v0.4
...
the champion is what \texttt{fish4/exact\_ii} models. Every exact figure in
...
that reached ``ours but we cannot place the split'' were nulled $23.4\%$ of the
time against $0.92\%$ otherwise, and accounted for $73\%$ of all nulls. Almost every
...
(\texttt{jobs/PREREGISTRATION\_retake\_bonus.md}), and it returns
