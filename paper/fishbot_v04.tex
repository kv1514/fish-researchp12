...
non-trivial. Whether the coupled form helps is still open, and the
attempt to settle it produced the more useful finding. Three runs of the
...
We do not report that as a result. It excludes zero by $0.04$; this configuration
has already produced one screening cell that failed to replicate; and reaching
for the pooling that happens to resolve, immediately after the pooling that
happened not to, is the same error in the opposite direction. The remedy for an
effect this size is not a better analysis of 1200 pairs. It is more pairs, at a
size chosen before looking: at $n = 2600$ the minimum detectable effect is $0.21$,
which separates $+0.23$ from $0$. That run is in progress and this section will
...
as a failure by choosing a run. Pooling all 1200 pairs gives $+0.226$, 95\% CI
$[+0.041, +0.411]$, which excludes zero by $0.04$ and is not enough to call after
this configuration has already failed to replicate once; a larger run is in
progress and Section~\ref{sec:res-lookahead} reports it. The implementation ships
disabled.
