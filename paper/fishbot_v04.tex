...
replicate twice. We also report several
...
\item \textbf{Search, once more, but differently.} Every search this project has
tried has sampled worlds, and the measured noise-to-signal ratio of $2.4$ across
sampled layouts is why they all failed. With exact marginals available, a
lookahead that operates on the \emph{belief} rather than on sampled
determinizations is deterministic given the belief, so the variance that killed
the previous attempts is structurally absent. That is the one search design this
