...
non-trivial. Settling whether the coupled form helps took 15{,}600 duplicate
...
what the second factor does. Across the $35$ cells of this study that store
their per-pair differentials it moves little in absolute terms --- $3.91 \pm
0.22$, a relative spread of $5.5\%$ against $9.0\%$ for the raw standard
...
estimate at these sample sizes is alone $3.0\%$, leaving about $4.6\%$ genuine
between-cell variation. \emph{The comparison is carried by a handful of cells}:
restricted to the $22$ cells with divergence share above $0.83$ --- where most of
this study lives --- the raw spread is $3.49\%$ and the conditional spread is
$3.48\%$, the same number, and the correlation between share and $\mathrm{sd}$
collapses from $+0.87$ to $+0.09$. Where share barely varies the decomposition
has nothing to explain, and it explains nothing. And \emph{the conditional term
is not constant}: it drifts with share, $\mathrm{corr} = +0.59$, fitted as
$2.48 + 1.72\,s$.
...
care. The calibration of the constant across a wide range of $s$ is \emph{not}
established here, because this study has almost no cells at low $s$.

The correlation between $s$ and the raw standard deviation is $+0.865$, and we
...
efficiency --- against $99.9$ with the opponent model disabled. The exact DP is
almost never the path taken: 9 decisions in 641 with the model on, and 14 in 646
with it off, so it is rare either way and the model additionally excludes it by
construction, since any $\gamma > 0$ changes the target the DP is built to
count. Either way the precision of every $\Prob(\text{success})$ in this engine
is the precision of that one sampled batch.
...
\paragraph{The policy can feel it, and it is the largest effect in this study.}
...
\]
a fit whose residuals stay within $0.03$\,ms across $40$ to $1920$ draws. At the
...
$2.55\,\mathrm{ms} + 8.0\,\mu\mathrm{s}$ per draw --- about $60\,\mu$s of fixed
...
with the interval clearing zero by $13\%$ of its own half-width, saying so costs
...
\end{tabular}
\end{center}
$95\%$ CI $[+0.285, +0.538]$ --- the largest effect in this project, comfortably
clear of zero, and roughly four times v0.3's best single term.
...
15{,}600 deal-pairs across four rounds to answer a question a 200-pair cell
appeared to answer at the start.
...
the whole study --- $+0.340$ sets per deal-pair --- comes from tripling the
sampling budget, which does not touch the objective at all: it only makes the
$\Prob(\text{success})$ the objective is already dominated by more accurate.
