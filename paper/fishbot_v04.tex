...
\paragraph{Not promoted, and why.} The flag is off. A best response is not an
equilibrium strategy: this measures that the policy beats \emph{the champion},
not that it plays Fish better, and Section~\ref{sec:noneq} is a standing
reminder that the two come apart. Turning it on would also change the champion,
and the champion is what \texttt{fish4/exact\_ii} models --- every exact figure
in Sections~\ref{sec:ii} and~\ref{sec:noneq} is a statement about the current
one. Promotion is a separate decision with a cascade behind it, not a
