...
efficiency --- against $99.9$ with the opponent model disabled. And the exact DP
is essentially never reached: 9 decisions against 632, because any $\gamma > 0$
changes the target and forces the sampling path. The precision of every
$\Prob(\text{success})$ in this engine is the precision of that one batch.
