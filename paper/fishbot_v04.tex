...
\title{\vspace{-1.6cm}\bfseries Exact Inference Is Not Enough:\\[0.1em]
\large Opponent Modelling, Exact Endgames, and Solver-Guided Policy Repair
in Six-Player Literature\vspace{-0.2em}}

\author{\normalsize Kausthubh Veldanda\\[-0.2em]
\small\texttt{kveldanda987@gmail.com}}
\date{\small 27 August 2026\vspace{-0.4em}}
...
\begin{abstract}
\small\setstretch{0.95}\setlength{\parskip}{0pt}\noindent
...
poker and the fully cooperative machinery developed for Hanabi. This paper is
the fourth iteration of an engine for it, organised around the results that
contradicted the expectations motivating them.
...
collapses onto a count matrix with at most nine rows. A dynamic program over
that structure returns the exact partition function, marginals, uniform samples
...
measurably \emph{worse} at predicting reality: over $517$ decisions and
$13{,}640$ uncertain-card predictions the exactly uniform posterior scored a
...
players' choices carry no information beyond legality --- and the sampler it
replaced had accidentally encoded a crude opponent model. Replacing the
...
$[+1.54, +2.17]$. Tripling the sampling budget is worth a further $+0.340$, more than three
times what belief-space search buys: the leverage in this game sits in the
belief rather than in what is computed from it.
...
half-suit live, $+0.3250$ at $35\%$ with two. Because two
thirds of the second layer is out of reach, we bound it rather than
extrapolate: a single-deal solve is the perfect-information relaxation and
...
$m = 1$); that move is an ask rather than a declaration; and it is
systematically riskier than the engine's ($p = 0.218$ against $0.799$, paired
$t = -18.88$, with the engine asking a card it was \emph{certain} to receive on
$73$ of $122$ positions and still being wrong). Fitting the ask objective
against exact one-ply values overturns the obvious diagnosis --- de-weighting
the success probability changes nothing --- and isolates certainty rather than
probability as the defect. The resulting one-parameter correction, pre-registered and replicated, gains
$\mathbf{+0.1220}$ $[+0.0711, +0.1729]$ sets on top of the configuration the
public engine actually plays. It does \emph{not} make that policy harder to
exploit: paired over $285$ positions, what a best response extracts moves by
$-0.0002$, half-width $0.026$. An exact best responder used as a critic
improves average-case play and leaves the worst case where it was. We also report two solver faults caught by controls needing no ground truth.

\vspace{1pt}
\noindent\textbf{Keywords:} imperfect-information games; exact inference;
opponent modelling; exploitability; endgame solving.
\end{abstract}
