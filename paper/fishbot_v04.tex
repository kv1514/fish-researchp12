...
fall out of the same table. Dropping terms by variance inflation removes
$\Prob(\text{success})$ \emph{first} and costs nothing measurable, so the term
this paper calls the objective adds no predictive value at these late positions
once the other ten are present. And every principal-component basis is far
worse, so the predictive directions in this design are not its high-variance
directions --- rotating to the biggest components discards exactly the signal.
