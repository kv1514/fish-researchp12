...
independently developed C++ engine running its own code --- it takes
$64.3\%$ of decided sets, and the corrected rule cuts nearly twice as hard
against the opponent ($306$ misdeclared sets to $160$); the endgame
correction, fitted against void-era targets, re-measures at $-0.104$
$[-0.218, +0.010]$ and is withdrawn pending a rule-matched refit.
...
so the tempo-versus-sets cancellation is no longer arithmetic that can be
assumed --- the duel is queued for re-measurement before ``buys no wins'' is
...
nearly twice as much to the opponent: v0.7's forced declarations come from
a sampled best guess where this engine declares the exact posterior MAP
(\S\ref{sec:claiming}), so scoring misdeclarations properly \emph{widened}
nothing --- it is the reason the margin held rather than shrank when the
game switched to the rule v0.7 was built for.
...
its misdeclarations about 20\% doing this in void-era measurement, and it
did not gain games there, because the turn it spent was worth about as much
as the sets it saved --- but that ledger was priced with a misdeclaration
worth one point of differential, and the award rule prices it at two, so
the trade has improved and its re-measurement is queued rather than
presumed. For a human, who is far more likely to misplace a split than an
engine is, the trade was probably favourable even at the old price.)
