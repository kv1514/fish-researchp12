...
\[
  3.52\,\mathrm{ms} \;+\; 5.84\,\mu\mathrm{s} \text{ per draw},
\]
a fit whose residuals stay within $\pm 0.21$\,ms across $40$ to $1920$ draws. At
the shipped $160$ the sampler is only a fifth of a decision, so tripling it costs
$+1.7$\,ms --- $1.35\times$, not $3\times$. That headroom is finite and the same
fit says so: by $1920$ draws a decision is already $3.2\times$ the shipped one.
...
Six lessons generalise beyond this game.
...
And it is not a lesson one learns once. The same error appears four times in this
...
ask objective. None was caught by being more careful. Each was caught by
measuring the quantity that had been reasoned about --- which is the next lesson,
and the reason it is stated separately.
