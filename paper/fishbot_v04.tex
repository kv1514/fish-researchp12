...
however much signal the target carries. That is consistent with what v0.4's fit
actually produced --- \texttt{turn} at $0.157$ where duplicate-deal play measures
$0.60$ --- and it says the re-run needs a different basis, or a penalty, or terms
dropped, and not merely more rollouts. The whole eleven-term fit explains
$11.7\%$ of within-position variance, so there is signal to work with; what there
is not is a way to attribute it to individual terms as they currently stand.
...
The situation is not rare, which is what makes the result usable: at $57\%$ of
decision points the acting seat is already two or more exchanges deep with some
opponent, and a re-take is available at $11\%$ of positions. So this is advice
...
worth at most --- $+0.098$ sets per deal-pair --- because the gate touches only
the $3\%$ of positions where a re-take is available and the duel is exactly one
deep, which is $29\%$ of what the ungated penalty flags.
