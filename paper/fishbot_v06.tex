...
in Six-Player Literature\\[0.15em]
\normalsize\normalfont FishBot v0.6\vspace{-0.5em}}
...
covers two iterations of an engine for it --- v0.4, which built the
inference, and v0.6, which corrected a rule and re-measured what depended on
it --- organised around the results that contradicted the expectations
motivating them.
...
$[+1.54, +2.17]$ (void-era scoring; restated below). Tripling the sampling budget is worth a further $+0.340$, more than three
...
$\mathbf{+2.375}$ sets per game, declaring correctly $96.7\%$ against
$78.9\%$, once a bridge defect of ours worth $-0.09$ to them is repaired;
...
This paper covers the fourth and sixth iterations of an engine for the game.
The bulk of it is v0.4, which built the inference and the evaluation
discipline; \S\ref{sec:critic} then reports v0.6, whose first act was to
discover that this engine had been playing a house variant of the
misdeclaration rule since v0.1 and whose remaining work was re-measuring, and
in two cases withdrawing, everything that had depended on it. The shipped
configuration is named \texttt{V06\_DEPLOYED} in the code and
\textbf{FishBot v0.6} on the site, and there is exactly one of it.
...
\texttt{results/award\_headline.json}; misdeclared-and-awarded sets $125$
against the deployed side, $146$ against v0.3).}
\label{tab:ladder}
\end{table}
...
not catch by reading its own results and nodding. Priced paired --- $4{,}000$
deals replayed under both bridge revisions, identical deals and identical
seating, the same duplicate-deal design every screen in this paper uses
(\path{scripts4/bridge_bug_price.py}) --- it is worth $\mathbf{-0.0915}$
$[-0.1076, -0.0754]$ sets per game: real, and about $4\%$ of the margin. It
...
Under the repair their declaration accuracy rises from $76.04\%$ to
$78.85\%$, while this engine's moves from $96.88\%$ to $96.69\%$ --- the
...
half-suits declared with the wrong split. Across the same $4{,}000$ paired
deals the allocation split is flat: $1{,}078$ against v0.7 before the
repair, $1{,}070$ after. The finding that this engine misdeclares roughly
...
\paragraph{The head-to-head, re-measured through the repaired bridge.} This
is the figure the paper stands behind, and the only one taken through bridge
revision~2: $4{,}000$ fresh games, seats rotated so neither engine opens more
often, both engines on their native misdeclaration rule, zero substituted
moves and zero unfinished games (\path{scripts4/mega_match.py} $\rightarrow$
\path{results/mega_match.json}).

\begin{center}
\begin{tabular}{lr}
\toprule
margin & $\mathbf{+2.3750}$ $[+2.2896, +2.4604]$ sets/game \\
sets & $22{,}750$ to $13{,}250$ ($63.19\%$ of decided) \\
games won & $80.70\% \pm 1.22$ ($3{,}228$W / $0$T / $772$L) \\
ask hit rate & $51.82\%$ against $48.18\%$ \\
declaration accuracy & $\mathbf{96.69\%}$ against $\mathbf{78.85\%}$ \\
\bottomrule
\end{tabular}
\end{center}
...
than either project's internal ladders: $+2.375$ $[+2.290, +2.460]$ sets per
game, $63.2\%$ of decided sets, over $4{,}000$ games under rules both engines
...
misdeclares \emph{more} ($123$ against $106$). The mechanism is not
