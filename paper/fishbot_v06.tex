...
it --- organised around results that contradicted the expectations motivating them.
...
$\mathbf{+2.347}$ sets per game, of which $\mathbf{61\%}$ is declaration
...
asserted from the rulebook. Deferring a declaration costs about $0.25$ to
$0.30$ sets each, and for the heavier arm that cost excludes zero --- a price
...
What makes the intercept causal rather than a selection artefact is the
pairing: both arms play the identical deal, so ``deals on which the error
count did not change'' is a property of the deal and not of an assignment.
...
\texttt{prereg/deadline\_signalling.md} registers exactly that, with the
incumbent $0.15$ carried as a replication arm --- if it does not reproduce the
$+0.068$ above, the new rung means nothing.
