"""prereg/tempo_regime.md: the ask objective charges for a turn that is free.

`askfeat.py` gives every candidate ask a tempo penalty of
`w_turn * (1 - p) * turn_risk[target]`, w_turn = 0.6, with no dependence on
what the turn is worth. The paper's tempo section measured that, bucketed by
exactly the same probability: a turn prices at -0.043 +- 0.169 below 0.25,
+0.004 +- 0.143 in [0.25, 0.50), and about +0.45 above 0.50. 57% of ask
decisions sit below that line (results/tempo_regime.json), and the objective
charges the full penalty at all of them.

THREE ARMS:

    A_shipped   the incumbent, one tempo rate everywhere
    B_free      the term switched off below p_best = 0.50
    C_half      half weight below 0.50, a dose rung whose job is to falsify:
                if it helps MORE than B, the threshold is not the one the
                tempo table points at and the story is wrong

The secondary outcomes are fixed by the pre-registration and reported whatever
the primary says: hit rate and asks per game (the term's direct target), TURNS
per game (because `scripts4/acquisition.py` found half our acquisition edge is
volume, and an intervention on tempo has to be read against volume), the
declaration path ledger, and the error-value decomposition.

    py scripts4/tempo_confirm.py [n_deals] [n_jobs]
"""
...
ARMS = {
    ...
    "C_half": {"turn_free_below": 0.50, "turn_free_scale": 0.5},
}
JOURNAL = Path(os.environ.get(
    "TEMPO_JOURNAL", ROOT / "results" / "tempo_journal.jsonl"))
