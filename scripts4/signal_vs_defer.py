...
def report(rows) -> dict:
    ...
    b, c, d = (contrast("B_signal", "A_shipped"),
               contrast("C_defer", "A_shipped"),
               contrast("D_both", "A_shipped"))
    print(f"\n  --- each against the shipped champion ---")
    for label, v in (("b  B_signal", b), ("c  C_defer", c), ("d  D_both", d)):
        print(f"    {label:14s} {fmt(*v)}")
    out["effects"] = {k: {"mean": v[0], "half_width": v[1], "ci95":
                          [v[0] - v[1], v[0] + v[1]], "n_clusters": v[2]}
                      for k, v in (("b", b), ("c", c), ("d", d))}

    # ---- withdrawal 1: B replicates, on BOTH uncertainties ----------------
    rm, rh = REPLICATE
    ...
    out["replication"] = {"target": rm, "target_half_width": rh,
                          ...
                          "passes": ok_rep}
    print(f"\n  REPLICATION GATE: B_signal {b[0]:+.4f} +-{b[1]:.4f} against "
          f"{rm:+.4f} +-{rh:.4f},\n    two-sample z = {z:+.2f} -> "
          f"{'PASS' if ok_rep else 'FAIL'}")

    # ---- primary: the interaction -----------------------------------------
