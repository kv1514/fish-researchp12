"""prereg/deadline_signalling.md: is the signalling gate set at the wrong place?

Not a new mechanism. The signalling protocol has been measured twice and
returned +0.002 [-0.086, +0.090] in the void era and +0.068 [-0.033, +0.169]
under the award rule, both at signal_max_p = 0.15, and it is switched off in
the deployed configuration.

What is at issue is the gate. It fires only when the ask the seat would
otherwise make has p <= 0.15, and that number was chosen before the paper
measured what a turn is worth. Bucketed by exactly that probability, a turn
prices at -0.043 +- 0.169 below 0.25, +0.004 +- 0.143 in [0.25, 0.50), and
about +0.45 above 0.50. A turn is free below 0.50. The gate sits at 0.15.

THREE ARMS:

    A_shipped   signalling off, the deployed champion
    B_incumbent signal_mode="stuck" at the default 0.15 -- a REPLICATION of
                r5_signal's +0.068. If this does not reproduce, C means
                nothing and the pre-registration says to withdraw the run.
    C_measured  signal_mode="stuck" at 0.50, the measured free-turn threshold

The secondary outcomes include the error-value decomposition fitted on this
run's own journal (scripts4/error_value.py). This project has twice reported
"the ledger moved and the margin did not" without being able to say why. It
can now, so a null here is required to be decomposed rather than filed.

    py scripts4/signal_gate_confirm.py [n_deals] [n_jobs]
"""
...
ARMS = {
    "A_shipped": {},
    "B_incumbent": {"signal_mode": "stuck"},
    "C_measured": {"signal_mode": "stuck", "signal_max_p": 0.50},
}
JOURNAL = Path(os.environ.get(
    "SIGNAL_JOURNAL", ROOT / "results" / "signal_gate_journal.jsonl"))
...
def report(rows) -> dict:
    ...
    print(f"\n=== the signalling gate, against v0.7 ===")
    print(f"{n:,} games ({n//2:,} deals x 2 parities), "
          f"each played once per arm on the identical deal\n")
    print(f"  arm A_shipped (signalling off)   {sum(base)/n:+.4f} sets/game")
...
def _load_journal():
    ...
    for i, line in enumerate(JOURNAL.read_text().splitlines(), 1):
        ...
        if not ROW_KEYS <= r.keys():
            raise SystemExit(
                f"{JOURNAL}:{i} is not a signal-gate row (keys present: "
...
def main(n_deals: int = 500, n_jobs: int = 0) -> int:
    ...
    if len(rows) < 80:
        ...
        return 1
    out = report(rows)
    dest = ROOT / "results" / "signal_gate_confirm.json"
